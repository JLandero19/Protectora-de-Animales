...
class CommentModel {
    ...
    public static function mdlWhere($table, $field, $value) {
        ...
        $query .= " AND status LIKE :condition2 ";

        $query .= " ORDER BY creation_date DESC ";
        
        $sql = Connect::connected()->prepare($query);
        $sql->execute(array(':condition' => $value, ':condition2' => "Valido"));
        $query = $sql->fetchAll();
    ...
    public static function mdlUpdateItem($table, $object, $id) {
        ...
        $sql = substr(trim($sql), 0, -1). " , modify_date = current_timestamp() WHERE id = :condition;";
